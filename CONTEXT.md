...
## PR-005 Release-candidate exercise (2026-08-19)
* **Status:** Final Claude audit returned `NOT APPROVED`; `v0.1.0-rc.1` at `81a157927fcacf4ae47716326885d12477e0002d` is permanently retired. No tag/release was created.
* **Provenance baseline:** `origin/main` `81a157927fcacf4ae47716326885d12477e0002d` (PR #70 / PR-004 merged). Dependabot PRs #39, #42, and #45 are closed.
* **Authority boundary:** Evidence exercise only. The label is not a Git tag; do not tag, publish, deploy, or create a release. Every external release action needs separate authorization.
* **Gates:** Claude’s 2026-08-19 second focused re-review returned `NO BLOCKERS`; MF-01–MF-09 and OQ-01/OQ-02 are resolved. Execution still requires active-session authority for an exact RC ID and its dereferenced 40-character SHA. Afterwards, exact-RC hosted evidence is required for all eight release cells and the docs job; local success is never a substitute. The final audit must directly confirm the actual `ci.yml` treatment of `tests/live/`, without relying on an inference from green CI. Windows Server evidence remains compatibility-only and does not close the Windows 11 desktop boundary.
* **Scope:** Evidence/documentation only. No runtime diagnostic, replay, telemetry, VIN, logging, or observability behavior changes.
* **Evidence:** Exact-SHA Actions run `32222492770` passed eight release cells (`647 passed` each) and docs (smoke `6 passed, 2 skipped`; regression `48 passed, 2 skipped`). Local isolated Node 24 docs build/smoke/regression passed; local CPython 3.11 is unsupported and supplies no package-cell evidence.
* **Final-audit findings:** (1) high-severity direct non-development `sharp <0.35.0` advisory without approved exception is the release blocker; (2) docs correctly condition Windows 11 support on native evidence and retention defaults fail closed, so neither independently blocks this candidate; (3) the PR-005 decision/release-note artifacts were untracked and absent from the authorized SHA, so were not auditable source. A new gated release/security follow-up must remediate `sharp`, add a Node dependency vulnerability gate to docs CI, commit/review PR-005 artifacts, use a new RC label/SHA, and collect a fresh full hosted run.
* **Next gate:** A Codex-owned pre-implementation remediation plan is in `docs/prompts/claude-pr-005-sharp-remediation-pre-audit.md`. No dependency, lockfile, workflow, or runtime change proceeds until Claude supplies a new `NO BLOCKERS` verdict for this narrower scope.
* **Pre-audit response:** Claude returned `MINOR FIXES` (MF-01 fail-open `continue-on-error`/shell-override bypass; MF-02 audit-threshold rationale; MF-03 explicit dev-dependency residual). The focused re-review packet is `docs/prompts/claude-pr-005-sharp-remediation-pre-audit-rereview.md`; implementation remains blocked pending `NO BLOCKERS`.
* **Focused re-review response:** Claude kept `MINOR FIXES` only for MF-01 because error suppression such as `set +e` or a custom step shell could bypass a bare audit command. The plan now forbids and contract-tests those forms in `docs/prompts/claude-pr-005-sharp-remediation-pre-audit-rereview-2.md`; implementation remains blocked pending its verdict.
* **Final plan re-review response:** Claude retained `MINOR FIXES` only because an audit-step `if:` could skip it and job-level `continue-on-error: true` could mask its failure. Both are now forbidden and contract-tested in `docs/prompts/claude-pr-005-sharp-remediation-pre-audit-rereview-3.md`; implementation remains blocked pending its verdict.
* **Remediation authorization:** Claude's final focused plan re-review returned `NO BLOCKERS`. The approved scope is `sharp` `^0.35.3`/npm lockfile regeneration, one fail-closed docs CI production audit step, its focused workflow-contract test, and committed PR-005 source artifacts only. A separate source-grounded implementation audit is required before any new RC is considered.
* **Remediation implementation:** Commit `3b95c1623b6d526d76f76ef00028a91efa595f1f` is pushed to `origin/pr-005-release-candidate-exercise` for remote source audit. Local Node 24 verification passed: production high-threshold audit has zero findings, docs build passed, smoke `6 passed, 2 skipped`, e2e `48 passed, 2 skipped`, and focused CI-contract tests `3 passed`. The actual-diff implementation-audit packet is `docs/prompts/claude-pr-005-sharp-remediation-implementation-audit.md`; no tag, release, or new RC exists pending Claude's verdict. The invalid nonfunctional `set -o +errexit` matcher was removed after audit feedback; macOS x64 installed-package license spot-check observed only the prior Apache-2.0/MIT/LGPL-3.0-or-later mix.
* **Implementation audit:** Claude source-verified `c648031ff3fba5f5a310a212f76526692f6515cc` and returned `APPROVED WITH MINOR FIXES`. Sharp/CI/test/source claims and adversarial `if:`, `set +e`, and job-level `continue-on-error` mutations passed audit. The sole MF is packet completeness: every changed file must be enumerated, including prompt archives, `CONTEXT.md`, and retired historical draft. The complete-manifest correction and focused re-review are in `docs/prompts/claude-pr-005-sharp-remediation-implementation-audit-rereview.md`; no runtime, release, or new RC action is authorized.
* **Disclosure re-review:** Claude source-verified `ddc3ad0eb458370348a5f1a4b1c8760cfb9baea0`, confirmed the manifest exactly matches the remediation diff, and returned `APPROVED FOR COMMIT`. All remediation-audit findings are closed. Hosted CI and ordinary PR review remain pending; the retired label is unchanged and no new RC/release action is authorized.
* **Hosted correction:** PR #76 run `32553311457` failed docs `npm ci` before the audit gate because a macOS npm 11 lock refresh pruned five sharp optional cross-platform records; all eight Python release cells passed but the run is invalid. The lock now adds only npm-generated Linux ppc64/riscv64 and Windows arm64 sharp records from a clean npm 10.9.2 generation, avoiding unrelated dependency drift. Corrected PR run `32554008022` is a single `pull_request` run at `23e6cb58ecbe37248e83bde46f059bae3025852d`: all eight package cells and the docs job passed, including `npm ci`, `npm audit --omit=dev --audit-level=high`, build, smoke, and e2e. The correction source audit remains pending in `docs/prompts/claude-pr-005-sharp-lockfile-correction-audit.md`; no RC/release action is authorized.
* **Correction-audit result (post-merge):** Claude source-audited the five cross-platform `sharp` lock records and returned `APPROVED FOR COMMIT`. It confirmed exactly five added lockfile package records, no pre-existing package version drift, live npm-registry integrity matches, and unchanged audited CI/contract-test scope. The audit occurred after PR #76 was squash-merged to `main` as `ae47d9d54a718cfe2bd3f5f2adfc632cb164790e`; that timing is a governance record, not retroactive authorization. Claude could not directly query the Actions API because of rate limits, but the independently verified corrected state is ancestor-identical to hosted run `32554008022`; separately, final PR-head run `32554209951` passed all nine jobs. No runtime, RC, tag, release, deployment, or Windows 11 claim is authorized.
