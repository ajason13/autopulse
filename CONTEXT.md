...
* **Hosted correction:** PR #76 run `32553311457` failed docs `npm ci` before the audit gate because a macOS npm 11 lock refresh pruned five sharp optional cross-platform records; all eight Python release cells passed but the run is invalid. The lock now adds only npm-generated Linux ppc64/riscv64 and Windows arm64 sharp records from a clean npm 10.9.2 generation, avoiding unrelated dependency drift. Local Node 24 `npm ci` and production audit pass. Re-audit packet: `docs/prompts/claude-pr-005-sharp-lockfile-correction-audit.md`; fresh hosted run remains required.
